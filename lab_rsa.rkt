...
(define (mod-exp b e m)
  (cond
    [(zero? e) 1]
    [(and (> e 0) (even? e)) (modulo (expt (mod-exp b (/ e  2) m) 2) m)]
    [else (modulo (* (mod-exp b (- e 1) m) b) m)])
  )



(define (mod-exp-help b e m)
   (modulo (exponent b e 1) m)
  )

(define (exponent b e ans)
  (if (eq? e 0)
      ans
      (exponent b (- e 1) (* ans b)))
  )
...
(define (encrypt plaintext key)
  (enc-help (string->list plaintext) (length (string->list plaintext)) (car key) (car (cdr key)) '())   
  )
(define (enc-help l len p k ans)
    (cond
      [(zero? len) ans]
      [else (enc-help (cdr l) (- len 1) p k (append ans (list (modulo (expt (char->integer (car l)) k) p))))])
  )
...
(define (decrypt ciphertext key)
  (list->string (map (lambda (a) (integer->char (mod-exp a (cadr key) (car key))) ) ciphertext))
  )
