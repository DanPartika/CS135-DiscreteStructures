...
(define (indegree-help E v ans)
   (cond
     [(null? E) ans]
     [(equal? (car (cdr (car E))) v) (indegree-help (cdr E) v (+ ans 1))]
     [else (indegree-help (cdr E) v ans)])
  )
...
;; 5 PTS
(define (find-top G)
   (find-top-help (get-edges G) (get-vertices G))
      )

(define (find-top-help G v)
     (cond
    [(null? v) (topsort-error)]
    [(zero? (indegree G (car v))) (car v)]
    [else (find-top-help G (cdr v))])
  )
...
(define (pop-vertex G v)
   (append (list (popv-vertices (get-vertices G) v)) (list (popv-edges (get-edges G) v)))
  )
...
;;need Graph to use previos function....
(define (topsort-help G ans)
  (cond
    [(null? (get-vertices G)) (reverse ans)]
    [else (topsort-help (pop-vertex G (find-top G)) (cons (find-top G) ans))])
  )