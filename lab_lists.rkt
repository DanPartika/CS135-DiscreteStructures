...
;; 2 PTS
(define (yoda words)
  (cons (car (cdr (cdr words))) (reverse (cdr (reverse words))))
  )
...
;; Type signature: (rotate '(1 2)) -> ((1 2) (2 -1) (-1 -2) (-2 1))
;; 2 PTS broken
(define (rotate c)
  (list c (reverse (list (* (car c) -1) (car (cdr c)))) (list (* (car c) -1) (* (car (cdr c)) -1)) (reverse (list (car c) (* (car (cdr c)) -1)))) 
  )
