...
(define (error-lcm)   (eopl:error "lcm error: input is negative"))
...
(define (wj-solvable? cA cB g)
  (cond
    [(and (> g cA) (> g cB)) #f]
    [(eq? (lcm g (gcd cA cB)) g) #t]
    [else #f]
   ;; [(eq? (lcm g (gcd cB g)) g)#t]
    )
  )
...
(define (waterjug cA cB g)
  (waterjug-help 0 0 cA cB g '())
  )

;; Type signature: (waterjug-help posint posint posint posint posint list) -> list of pairs
(define (waterjug-help a b ma mb goal ans)
  (cond
    [(eq? b goal) (reverse (cons (list a b) ans))]
    [(eq? b mb) (waterjug-help a 0 ma mb goal (cons (list a b) ans))]
    [(zero? a) (waterjug-help ma b ma mb goal (cons (list a b) ans))]
    [(> (+ a b) mb) (waterjug-help (- a (- mb b)) mb ma mb goal (cons (list a b) ans))]
    [(<= (+ a b) mb) (waterjug-help 0 (+ a b) ma mb goal (cons (list a b) ans))])
  )
...
(define (factorize n)
  (cond
    ...
    [(zero? n) '(0)]
    [(eq? n 1) '(1)]
    [else (reorder (factorize-help n 2 '()) '())])
  )
;;original list, length of list , answer list --> list
(define (reorder l ans)
  (cond
    [(zero? (length l)) ans]
    [else (reorder (cdr l) (cons (car l) ans))])
  )

(define (factorize-help n p ans)
  
  (cond
    [(eq? n 1) ans]
    [(divides? p n) (factorize-help (/ n p) p (cons p ans))]
    [else (factorize-help n (+ p 1) ans)])
...
;; 3 PTS
(define (cong-mod? m)
  (lambda (a b) (cong-mod-help m a b) ))

  (define (cong-mod-help m a b)
    (eq? (modulo a m) (modulo b m))
    )
  ;;((cong-mod-help m) a b)
