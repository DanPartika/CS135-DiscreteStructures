...
;; 5 PTS
(define (compose S R)
  (define  (composeHelp R S)
    (map (lambda (i) (list (car (car R)) i)) (relate (cadar R) S)))
  (if (null? R)
      '()
      (union (composeHelp R S) (compose S (cdr R))))
  )
...
(define (powerHelp n R S)
  (if (equal? n 0)
      '()
      (if (equal? n 1)
          S
          (powerHelp (- n 1) R (compose R S))))
  )
...
(define (transitive-closure R)
  (transitive-closureHelp R R (length R))
  )

(define (transitive-closureHelp R T L)
  (if (equal? L 0)
      '()
      (if (equal? L 1)
          T
          (union T (transitive-closureHelp R (power L R) (- L 1) ))))
  )
